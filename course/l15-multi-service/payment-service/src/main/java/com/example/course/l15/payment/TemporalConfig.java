...
public class TemporalConfig {
    ...
    /** Polls only the "payment-service" queue and knows only PaymentActivity. No workflow is registered here. */
    @Bean
    public Worker paymentWorker(WorkerFactory factory,
                                PaymentActivity paymentActivity) {
        Worker worker = factory.newWorker("payment-service");
        worker.registerActivitiesImplementations(paymentActivity);
        return worker;
    }

    @Bean
    public ApplicationRunner startWorkerFactory(WorkerFactory workerFactory, Worker paymentWorker) {
